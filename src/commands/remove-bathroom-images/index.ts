...
export default class RemoveBathroomImagesSlashCommand extends SlashCommand {
    public static readonly data = removeBathroomImagesData;
    public static readonly documentation = removeBathroomImagesDocumentation;

    private readonly COLLECTOR_TIME = 60000 * 5; // 5 minutes
    private readonly collectorOptions: MessageCollectorOptionsParams<MessageComponentType, boolean> =
        { filter: (i) => i.user.id === this.interaction.user.id, time: this.COLLECTOR_TIME };
    private remainingUrls: string[] = [];
    ...
    async controlImageSelection(this: RemoveBathroomImagesSlashCommand, imageUrl: string, index: number) {
