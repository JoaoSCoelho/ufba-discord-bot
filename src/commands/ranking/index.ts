...
export default class RankingSlashCommand extends SlashCommand {
    ...
    public async execute() {
        ...
        const guildMembers = this.client.database!.member.filter((member) => member.discordGuildId === this.interaction.guildId);
        ...
        const targetUser = this.interaction.options.get('membro')?.user ?? this.interaction.user;
        ...
        const rankingLines = await Promise.all(top15Members.map(async (member, index) => {
            const discordMember = await this.client.guilds.cache.get(this.interaction.guildId!)?.members.fetch(member.discordId)
                .catch((error: unknown) => {
        ...
        await this.interaction.followUp(`\`\`\`ansi\n${rankingLines.join('\n')}\n\`\`\`\n${targetUser.id === this.interaction.user.id ? 'Sua posição' : `A posição de ${targetUser}`} no ranking é **${userPositionInRanking}º**`);
    }
};