...
export default class RegisterBathroomSlashCommand extends SlashCommand {
    public static readonly data = registerBathroomData;
    ...
    private defaultCollectorFilter(i: Parameters<CollectorFilter<[StringSelectMenuInteraction<CacheType> | UserSelectMenuInteraction<CacheType> | RoleSelectMenuInteraction<CacheType> | MentionableSelectMenuInteraction<CacheType> | ChannelSelectMenuInteraction<CacheType> | ButtonInteraction<CacheType>]>>[0]) {
    ...
    private async bathroomPreviewMessageOptionsFactory(bathroomData: Omit<{ [K in keyof Bathroom]: Bathroom[K] | null }, 'createdAt' | 'updatedAt' | 'createdBy' | 'id'>) {
        return {
            embeds: [await bathroomEmbedFactory(this.client, {
                createdAt: new Date(),
                updatedAt: new Date(),
