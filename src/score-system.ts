...
export default async function scoreSystem(client: LocalClient, message: Message<true>) {
    ...
    const dbMember = client.database.member.find((member) => 
        member.discordId === message.member!.id && member.discordGuildId === message.guild.id
    );

    if (!dbMember) {
        log.loadingh(`Adicionando membro #(@${message.member.user.tag})#`, 
            `do servidor #(${message.guild.name})# ao banco de dados`);
        ...
        await client.database.member.new(newMember)
            .then(() => {
                log.successh(`Membro #(@${message.member!.user.tag})#`, 
                    `do servidor #(${message.guild.name})# adicionado ao banco de dados`, 
                    `com #(${newMember.score})# pontos de score`);
            })
            .catch((error) => {
                log.error(`Erro ao adicionar membro #(@${message.member!.user.tag})#`, 
                    `do servidor #(${message.guild.name})# ao banco de dados`,
                    '\n#(Membro)#:', newMember, 
                    '\n#(Error)#:', error);
            });
    } else {
        log.loadingh(`Adicionando #(3)# pontos de score ao membro #(@${message.member.user.tag})#`, 
        ...
        await client.database.member.edit(newMemberData)
            .then(() => {
                log.successh(`Membro #(@${message.member!.user.tag})#`, 
                    `do servidor #(${message.guild.name})# recebeu #(3)# pontos de score`, 
                    `e possui agora #(${newMemberData.score})# ao total`);
            })
            .catch((error) => {
                log.error(`Erro ao adicionar score ao membro #(@${message.member!.user.tag})#`, 
                    `do servidor #(${message.guild.name})#`,
                    '\n#(Data)#:', newMemberData, 
                    '\n#(Erro)#:', error);
            });
    ...
    // Verifies if the member has been passed to the next level
    ...
    levels.forEach(({ targetScore }, index) => {
        if (dbMemberScore < targetScore && member.score >= targetScore) {
            log.infoh(`O membro #(@${message.member?.user.tag ?? message.author.id})#`, 
                `do servidor #(${message.guild.name})# passou para o nível #(${index + 1})#`);
            ...
            message.reply(messageContent)
                .catch((error) => {
                    log.error(`Erro ao enviar resposta "#(${messageContent})#"`,
                        `para o usuário #(@${message.author.tag})#`, 
